...
- Type guards for narrowing content entries: `isFormbarControl`, `isFormbarGroup`, `isFormbarArray`, `isFormbarBlock`. Each accepts either a value or a `Signal` and narrows to the corresponding `NgxFbControl` / `NgxFbFormGroup` / `NgxFbArray` / `NgxFbBlock` shape
...
- Peer dependencies are now limited to `@angular/core` (`>=20.0.0 <22.0.0`) and `rxjs` (`>=7.0.0`). `@angular/forms` and `@angular/cdk` are no longer required by the core package
...
- Type renamed: `NgxFbContent` is now `NgxFbItem` (`NgxFbFormGroup | NgxFbControl | NgxFbArray | NgxFbBlock`). Update any imports or type annotations that referenced `NgxFbContent`
