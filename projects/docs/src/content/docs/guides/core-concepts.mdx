...
This page describes the different concepts and how everything works. It is not required to understand this for normal usage, but valuable in case you configure _ngx-formwork_ by yourself or just are curios.

## Registration Types

In _ngx-formwork_ there are two ways to register controls and validators: `token` and `map`.
...
If you provide an object based registration to the `provideFormwork` function and in also add the token to the providers array, there are two possible outcomes:
- You first call `provideFormwork` and then add the token: The registrations from the token win
- You first add the token, then call `provideFormwork`: The registrations from the object win
